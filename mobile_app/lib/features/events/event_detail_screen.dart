...
class EventDetailScreen extends StatelessWidget {
  ...
  String _eventShareUrl() {
    final fromApi = (event['share_url'] ?? '').toString().trim();
    if (fromApi.isNotEmpty && fromApi.toLowerCase() != 'null') {
      return fromApi;
    }

    final eventId = event['id'];
    final title = (event['title'] ?? 'event').toString();
    final slug = _slugify(title).isNotEmpty
        ? _slugify(title)
        : 'event-$eventId';
    const configuredShareBase = String.fromEnvironment(
      'PUBLIC_SHARE_BASE_URL',
      defaultValue: 'https://www.femalefoundersinitiative.com',
    );
    final shareBase = configuredShareBase.trim().replaceAll(RegExp(r'/$'), '');
    return '$shareBase/share/events/$eventId/$slug/';
  }

  String _buildShareText() {
    final title = (event['title'] ?? 'Event').toString().trim();
    final date = (event['date'] ?? '').toString().trim();
    final location = (event['location'] ?? '').toString().trim();
    final url = _eventShareUrl();

    final meta = [date, location].where((v) => v.isNotEmpty).join(' • ');
    if (meta.isEmpty) {
      return '$title\n$url';
    }
    return '$title\n$meta\n$url';
  ...
  Future<void> _shareEvent(BuildContext context) async {
    ...
    try {
      await SharePlus.instance.share(
        ShareParams(
          text: text,
          subject: (event['title'] ?? 'Event').toString(),
        ),
      );
    } catch (_) {
  ...
  void _onRSVP(BuildContext context) async {
    ...
    if (tiers == null || tiers.isEmpty) {
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(
          content: Text("No RSVP options available for this event."),
        ),
      );
      return;
    }
                              ...
                              Expanded(
                                child: ElevatedButton(
                                  ...
                                  style: ElevatedButton.styleFrom(
                                    ...
                                    elevation: 8,
                                    shadowColor: FfigTheme.primaryBrown
                                        .withOpacity(0.4),
                                  ),
                                  child: Text(
                                    "CONFIRM",
    ...
    return TextFormField(
      ...
      decoration: InputDecoration(
        ...
        labelStyle: GoogleFonts.inter(
          color: isDark ? Colors.white.withOpacity(0.5) : FfigTheme.textLight,
          fontSize: 13,
        ),
  ...
  bool _isConcluded() {
    ...
    try {
      final dateStr =
          (event['end_date'] != null && event['end_date'].toString().isNotEmpty)
          ? event['end_date']
          : event['date'];
      ...
      child: Scaffold(
                    ...
                    title: LayoutBuilder(
                      builder: (context, constraints) {
                        ...
                        final double opacity =
                            (1.0 - (top - collapsedH) / 100.0).clamp(0.0, 1.0);

                        return Opacity(
                          opacity: opacity,
                          child: Text(
                            event['title'].toString().replaceAll(
                              RegExp(r'poala', caseSensitive: false),
                              'Paola',
                            ),
                            style: TextStyle(
                              ...
                              children: [
                                Text(
                                  ...
                                  style: TextStyle(
                                    ...
                                    fontSize:
                                        20, // Slightly smaller for better fit
                                    fontWeight: FontWeight.w900,
                                    letterSpacing: 1.2,
                                  ),
                                  maxLines: 2,
        ...
        bottomNavigationBar: SafeArea(
          child: Padding(
            ...
            child: ElevatedButton(
              onPressed: (event['is_sold_out'] == true || _isConcluded())
                  ? null
                  : () => _onGetTickets(context),
              style: ElevatedButton.styleFrom(
                padding: const EdgeInsets.symmetric(vertical: 16),
                backgroundColor:
                    (event['is_sold_out'] == true || _isConcluded())
                    ? Colors.grey
                  ...
                  Container(
                    ...
                    child: Row(
                      ...
                      children: [
                        ...
                        Text(
                          () {
                            if (event['ticket_tiers'] != null &&
                                (event['ticket_tiers'] as List).isNotEmpty) {
                              try {
                                final tiers = event['ticket_tiers'] as List;
                                ...
                                for (var t in tiers) {
                                  double p =
                                      double.tryParse(t['price'].toString()) ??
                                      0.0;
                                  if (p < minP) {
                                    minP = p;
                                    cheapest = t;
                                ...
                                final cur = (cheapest['currency'] ?? 'usd')
                                    .toString()
                                    .toUpperCase();
                                return minP == 0
                                    ? "Free"
                                    : "$cur ${minP.toStringAsFixed(2)}";
                              } catch (e) {
                ...
                delegate: SliverChildBuilderDelegate((context, index) {
                  ...
                  return Container(
                    ...
                    decoration: BoxDecoration(
                      color: Theme.of(
                        context,
                      ).cardColor, // Use cardColor for responsive background
